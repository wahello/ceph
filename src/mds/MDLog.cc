...
void MDLog::standby_trim_segments()
{
  ...
  while (have_any_segments()) {
    ...
    if (seg->end > journaler->get_read_pos()) {
      dout(10) << " won't remove, still reading!" << dendl;
      // This is an obscure case where the expire pos has proceeded
      // past the end of the segment, but we (the standby replayer)
      // are currently replaying the region of the journal between
      // trimmed_pos and expire pos.
      break;
    }
